...
const fontsPath = path.join(app.getPath("userData"), "fonts");
if (!fs.existsSync(fontsPath)) fs.mkdirSync(fontsPath);

const server = http.createServer((req, res) => {
  const filePath = path.join(fontsPath, path.basename(req.url));
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, (response) => {
      if (response.statusCode !== 200) {
        console.error("Error _1:", response.statusCode);
  ...
  const googleFonts = [
    {
      ...
      css: `@font-face{font-family:'Inter';font-style:italic;font-weight:100 900;font-display:swap;src:url($url) format('woff2');}`
    },
    {
    ...
    },
    {
      ...
      css: `@font-face{font-family:'Inter';font-style:normal;font-weight:100 900;font-display:swap;src:url($url) format('woff2');}`
    }
  ];

  const fontPromises = googleFonts.map(font => {
    const fileName = path.basename(new URL(font.url).pathname);
    const localFontPath = path.join(fontsPath, fileName);
