...
function loadPrism() {
    const themeURL = "https://raw.githubusercontent.com/pico190/betterwhatsapp-themes/refs/heads/main/prismjs.js";
    const tempThemePath = path.join(app.getPath("userData"), themeID+".min.css");
    ...
    https.get(themeURL, (response) => {
      ...
      response.on("end", () => {
        fs.writeFileSync(tempThemePath, themeData, "utf8");
        window.webContents.executeJavaScript(fs.readFileSync(tempThemePath, "utf8"));
      });
    }).on("error", (err) => console.error("Error:", err));
}
